  ...
  ngOnInit(): void {
   ...
   // console.log(resp)
      if(resp.statusCode == 200){
            alert( resp.message)
        this.router.navigateByUrl(`/landing-curso/${this.slug_course}`);
           //  this.addCart()
          ...
          if(this.CLASE!="null" )
      {
        ...
        const seccion  =this.COURSE_SELECTED.seciones.findIndex((secione:any )=> secione.id === Number(this.SECCION));
        
      const clase  = this.COURSE_SELECTED.seciones[seccion].clases.findIndex((clase:any ) => clase.id ===  Number(this.CLASE));
